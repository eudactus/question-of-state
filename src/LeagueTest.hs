...
main = hspec $ do
    ...
    describe "calculateScores" $ do
        it "can score for one result" $ do
            calculateScores [(MatchResult celtic 1 liverpool 0)] `shouldBe` [(celtic, 2), (liverpool, 0)]
        it "can score for two results" $ do
            calculateScores [(MatchResult celtic 1 liverpool 0), (MatchResult celtic 1 plymouthArgyle 1)] `shouldBe` [(celtic, 2), (liverpool, 0)]            
